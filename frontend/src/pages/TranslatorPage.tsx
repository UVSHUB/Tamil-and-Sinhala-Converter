      ...
      if (prevSessionState.current === 'AI_SPEAKING' || prevSessionState.current === 'AI_LISTENING') {
        ...
        if (userText || aiText) {
          ...
          setHistory(prev => {
            ...
            if (lastMsg && ((userText && lastMsg.text === userText) || (aiText && lastMsg.text === aiText))) {
              return prev;
            }
            return [...prev, ...newMessages];
          });
          // Note: We retain sourceCaption and targetCaption on screen
          // so the user can clearly see and read the translated speech.
        }
      }
    }
    prevSessionState.current = sessionState;
  }, [sessionState, sourceCaption, targetCaption, sourceLang, targetLang]);
    ...
    <div className="flex flex-col h-screen bg-slate-950 overflow-hidden font-sans relative">
      ...
      <header className="flex items-center justify-between px-5 py-3 border-b border-slate-800 shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Speaker Sound Test Button */}
          <button
            onClick={() => playTestSound()}
            className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-indigo-500/30 bg-indigo-500/10 text-indigo-300 hover:bg-indigo-500/20 active:scale-95 text-xs font-semibold transition-all shadow-sm"
            title="Click to test your laptop speakers with a test chime"
          >
            <Volume2 className="h-3.5 w-3.5 text-indigo-400" />
            <span className="text-[11px] font-medium hidden sm:inline">Test Sound</span>
          </button>
      ...
      <div className="flex-1 relative px-5 pb-4 min-h-0 overflow-hidden flex gap-4">
        ...
        <div className="flex-1 flex flex-col items-center justify-between bg-slate-950 border border-slate-800 rounded-2xl py-6 px-6 shadow-2xl shadow-black/50 relative overflow-hidden">
          ...
          {/* Waveform canvas — positioned underneath elements */}
          <div className="absolute left-0 right-0 w-full h-28 bottom-24 z-0 pointer-events-none">
            <canvas ref={canvasRef} className="w-full h-full" style={{ background: 'transparent' }} />
          </div>

          {/* Top: status badge & voice mode */}
          <div className="flex flex-col items-center gap-1.5 w-full z-10 shrink-0">
            <div className={`flex items-center gap-1.5 text-[10px] font-extrabold uppercase tracking-widest px-3 py-1 rounded-full border transition-all duration-300 ${
              ...
              {sessionState === 'AI_LISTENING' && 'Listening to Speech'}
              {sessionState === 'AI_THINKING' && 'Translating Speech'}
              {sessionState === 'AI_SPEAKING' && 'Speaking Translation'}
            ...
            {sessionState === 'AI_LISTENING' && (
              <div className="text-[9px] font-bold uppercase tracking-widest text-slate-500">
                {detectedGender ? (
                  ...
                  <span className="text-slate-600 animate-pulse">🎤 Listening for voice...</span>
                )}
              </div>
            )}
          </div>

          {/* ── LIVE TRANSLATION & SUBTITLE DISPLAY ── */}
          <div className="w-full max-w-xl z-10 my-auto flex flex-col gap-3">
            {/* User speech card */}
            <div className="bg-slate-900/90 backdrop-blur border border-slate-800/80 rounded-2xl p-4 shadow-lg flex items-start gap-3 transition-all">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  ...
                  {sessionState === 'AI_LISTENING' && sourceCaption && (
                ...
                <p className="text-base text-slate-100 font-medium leading-relaxed break-words">
                  {sourceCaption || (
                    <span className="text-slate-500 italic text-sm">
                      {sessionState === 'AI_LISTENING' ? 'Speak in Sinhala, Tamil, or English...' : 'Tap the microphone below to start speaking'}
                    </span>
                  )}
                </p>
              </div>
            ...
            {/* Translation card */}
            <div className={`bg-gradient-to-br from-indigo-950/70 via-slate-900/90 to-slate-950 border rounded-2xl p-5 shadow-2xl flex items-start gap-3 transition-all duration-300 ${
              sessionState === 'AI_SPEAKING' ? 'border-indigo-500/60 shadow-indigo-500/20 ring-1 ring-indigo-500/40' : 'border-slate-800'
            }`}>
              <div className={`p-2.5 rounded-xl shrink-0 mt-0.5 transition-all ${
                sessionState === 'AI_SPEAKING' ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/30 animate-pulse' : 'bg-indigo-500/10 border border-indigo-500/20 text-indigo-400'
              }`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[10px] font-extrabold uppercase tracking-widest text-indigo-400 flex items-center gap-1.5">
                    SinTam Translation ({targetLang})
                    {targetCaption && (
                      <button onClick={() => handleCopyText(targetCaption, 'live-target')} className="text-slate-500 hover:text-white p-0.5 transition-colors" title="Copy translation">
                        {copiedId === 'live-target' ? <Check className="h-3 w-3 text-emerald-400" /> : <Copy className="h-3 w-3" />}
                      </button>
                    )}
                  </span>
                ...
                <p className="text-xl sm:text-2xl text-white font-bold tracking-wide leading-relaxed break-words">
                  {targetCaption || (
                    <span className="text-slate-500 font-normal italic text-sm">
                      Translated speech will appear and be spoken out loud here
                    </span>
                  )}
                </p>
              </div>
          ...
          {/* Center: Mic button */}
          <div className="flex flex-col items-center gap-2 z-10 shrink-0 my-2">
            <div className="relative flex items-center justify-center">
              ...
              <button
                ...
                className={`h-24 w-24 rounded-full flex items-center justify-center text-white transition-all duration-300 shadow-2xl relative z-10 hover:scale-105 active:scale-95 ${
                  sessionState === 'IDLE'
            ...
            <p className="text-xs text-slate-500 font-medium tracking-wide">
              {sessionState === 'IDLE' ? 'Tap microphone to start' : 'Tap microphone to stop'}
            </p>
          </div>

          {/* Bottom: Quick text input & actions */}
          <div className="w-full max-w-lg z-10 shrink-0 flex items-center gap-2">
            <form onSubmit={handleSendText} className="flex-1 flex items-center gap-2 bg-slate-900/90 border border-slate-800 rounded-xl px-3 py-1.5 shadow-lg">
              <input
                type="text"
                value={inputText}
                onChange={e => setInputText(e.target.value)}
                placeholder="Or type Sinhala / Tamil / English..."
            ...
            <button
              ...
              disabled={history.length === 0 && !sourceCaption && !targetCaption}
              ...
              <div className="flex items-center justify-between text-[11px] text-slate-500 font-semibold">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => playTestSound()}
                    className="text-[10px] px-2 py-0.5 rounded bg-indigo-500/20 text-indigo-300 hover:bg-indigo-500/30 border border-indigo-500/30 font-medium transition-all"
                  >
                    🔊 Test Volume
                  </button>
                </div>
